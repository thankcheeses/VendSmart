...
import { dailyRevenue, productPerformance } from '@/data/mockData';
...
export default function AnalyticsPage() {
  ...
  const avgDaily = Math.round(totalRevenue / dailyRevenue.length);
  const bestDay = dailyRevenue.reduce((a, b) => (a.total > b.total ? a : b));
  // const worstDay = dailyRevenue.reduce((a, b) => (a.total < b.total ? a : b));

  const topProducts = [...productPerformance].sort(
    (a, b) => b.revenue_7d - a.revenue_7d
  );
  const underperforming = productPerformance.filter(
    (p) => p.units_sold_7d < 45
  );
        ...
        <div className="glass-card">
          ...
          <div className="flex items-center gap-5 mt-3 justify-center">
            {[
              ...
              { label: 'Drink', color: '#22D3EE' },
              { label: 'Snack', color: '#22C55E' },
              { label: 'Combo', color: '#F59E0B' },
            ].map((item) => (
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      <div
                        ...
                        style={{ color: 'var(--text-secondary)' }}
                      >
                        {p.category} · {p.units_sold_7d} sold this week
                      </div>
                    </div>
                  </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          <div className="glass-card lg:col-span-2">
            <h3
              ...
              style={{ color: 'var(--text-primary)' }}
            >
              Revenue per Machine Type
            </h3>
            <ResponsiveContainer width="100%" height={280}>
              <BarChart
                data={[
                  { type: 'Snack', avgRev: 248, machines: 5 },
                  { type: 'Drink', avgRev: 312, machines: 4 },
                  { type: 'Combo', avgRev: 289, machines: 3 },
                ]}
              >
                <CartesianGrid
                  strokeDasharray="3 3"
                ...
                <Tooltip
                  ...
                  formatter={(value: number, name: string) => [
                    name === 'avgRev' ? `$${value}` : value,
                    name === 'avgRev' ? 'Avg Revenue' : 'Machines',
                  ]}
                />
